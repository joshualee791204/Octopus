# Idea borrowed from this script: Idea borrowed from this script: https://www.kaggle.com/serigne/stacked-regressions-top-4-on-leaderboard


def impute1(all_data):
	...
	#all_data['TotalFinSF'] = all_data['TotalBsmtSF'] + all_data['1stFlrSF'] + all_data['2ndFlrSF'] - all_data['2ndFlrSF']
	...
	all_data["LotFrontage"] = all_data.groupby('HouseStyle')["LotFrontage"].transform(lambda x: x.fillna(x.median()))
...
def impute(all_data):
	...
	all_data['FullBath'] = all_data['FullBath'].astype(float)
	all_data['FullBath'] = all_data['FullBath']+all_data['HalfBath']*0.5
	...
	all_data = all_data.drop(['BsmtCond'], axis=1)
	...
	# LotFrontage : Since the area of each street connected to the house property most likely have a similar area to other houses in its neighborhood , we can fill in missing values by the median 	 of the neighborhood.
	# Group by neighborhood and fill in missing value by the median LotFrontage of all the neighborhood
	all_data["LotFrontage"] = all_data.groupby('HouseStyle')["LotFrontage"].transform(lambda x: x.fillna(x.median()))
